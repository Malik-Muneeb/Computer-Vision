...
def nms(A, brd):
    ...
    for i in range(brd, A.shape[0] - brd):
        for j in range(brd, A.shape[1] - brd):
            # Find Alm[i,j] which is either 0 or A[i,j] in case of local maxima
            localMaxima = A[i-brd][j-brd]
            for a in range(i - brd, i + brd + 1):
                for b in range(j - brd, j + brd + 1):
                    if A[a, b] > localMaxima:
                        localMaxima = A[a, b]

            if A[i, j] == localMaxima:
                Alm[i, j] = localMaxima
...
#compute cornerness
det = (dx_sq * dy_sq)-(dxdy*dxdy) #determinant of structure tensor at every pixel
